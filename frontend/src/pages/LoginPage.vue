      ...
      <div class="card-body p-4">
        ...
        <form @submit.prevent="submit">
          <div class="mb-3">
            ...
            <input v-model="name" type="text" class="form-control auth-input" required autofocus autocomplete="username" />
          </div>
          <div class="mb-3">
            <label class="form-label">{{ t("password") }}</label>
            <input v-model="password" type="password" class="form-control auth-input" required autocomplete="current-password" />
          </div>
          ...
          <button :disabled="loading" type="submit" class="btn btn-danger w-100">{{ loading ? t("loading") : t("login") }}</button>
        </form>
        <div class="text-center mt-3">
          <router-link class="text-white-50" to="/register">{{ t("no_account_register") }}</router-link>
        </div>
      </div>
    </div>
  </div>
...
const submit = async () => {
  ...
  error.value = "";
  try {
    await auth.login({ name: name.value, password: password.value, remember: remember.value });
...
onMounted(() => {
  remember.value = localStorage.getItem("owazym_remember_me") !== "0";
  applyCurrentTheme();
  bindAuthInputFixes();
